...
class PayersBlock(BaseXlsBlock):
    ...
    def write_data(self):
        ...
        clients_payments.sort(key=lambda x: datetime.strptime(x['payment_created_at'], '%Y-%m-%dT%H:%M:%S.%fZ'), reverse=True)

        quarters = {}
        for client_payment in clients_payments:
            payment_date_str = client_payment['payment_created_at'][:-1]  # Удаляем "Z" в конце строки
